...
int main() {
    ...
    const auto mask_exp2 = vsl::cxm::about_equal(res_exp2, ref_exp2, tol);
    assert(vsl::elements_equal(mask_exp2, true_mask));

    // log2
    const auto arg_log2 = vsl::float4(69.f);
    const auto res_log2 = vsl::cxm::log2(arg_log2);
    const auto ref_log2 = vsl::float4(std::log2(69.f));
    const auto mask_log2 = vsl::cxm::about_equal(res_log2, ref_log2, tol);
    assert(vsl::elements_equal(mask_log2, true_mask));
